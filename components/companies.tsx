...
export const Companies = () => {
  ...
  return (
    <div className="relative z-20 py-10 md:py-40">
      ...
      {/* Add the full-width background here */}
      <div className="relative mt-20 bg-white/10 dark:bg-white/10 backdrop-blur-md px-10 py-10 rounded-lg w-full">
        <div className="flex gap-10 flex-wrap justify-center md:gap-40 relative h-full w-full">
          <AnimatePresence
            mode="popLayout"
            onExitComplete={() => {
          ...
          >
            {activeLogoSet.map((logo, idx) => (
              <motion.div
                ...
                key={logo.title}
                className="relative dark:animate-glow"
              >
                <Image
                  src={logo.src}
                  alt={logo.title}
                  width="100"
                  height="100"
                  className="md:h-20 md:w-40 h-10 w-20 object-contain"
                />
              </motion.div>
            ))}
          </AnimatePresence>
